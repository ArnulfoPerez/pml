...
inTraining <- createDataPartition(data$classe, p = 0.9, list = FALSE)
...
The caret function trainControl allows one to set the training sequence. Here, I am setting 3 cycles of 10-fold cross validation, set arbitrarily from typical values.
...
The performance of the Random Forest classifier with repeated cross validation is very good. Just for fun, let's try another control method:

```{r randomForest_oob, cache=TRUE}
set.seed(12345)
modelRF2 <- train(classe ~ ., data=trainData, method="rf", trControl=trainControl(method='oob'))
modelRF2$finalModel
plot(modelRF2)
```
